...
\begin{abstract}
This paper provides a technique for solving discrete time  dynamic stochastic
infinite-horizon economies with bounded-time-invariant solutions.
In addition, the paper provides 
formulae for evaluating the accuracy of decision rules
approximately satisfying the system of model equations and presents
an algorithm that robustly improves the accuracy of proposed solutions.
The error bounds apply for assessing the accuracy 
of any proposed bounded time invariant decision rule, 
...
To speed large K computations check for convergence and use 
...
\label{sec:newseries}
Since \cite{blanchard80} numerous authors have developed solution algorithms and formulae for solving linear rational expectations models.  The solutions associated with these linear saddle point models turn out to be instrumental in characterizing the solutions for highly nonlinear models.
...
\end{gather}
It will be useful to collect the components of this solution for use later in the paper.
...
We will use these matrices to construct a series representation for any bounded path.
\begin{theorem}
...
\end{gather}
One can then express the $\xWOarg_t$ solving \ref{hSystem} along with the anticipated shocks $\zWOarg_t$ by the equation
	 \begin{gather}
...
%The formula will provide a series  for any $L$ dimensional $\linMod$.
 The formula supports the intuition that far distant shocks influence current conditions less than  imminent shocks.



% \footnote{$\psi_c$ full rank provides a sufficient condition, but not necessary condition.}
The
transformation $ {\{ x_{t}, x_{t}, x_{t+1},x_{t+2}\ldots\}} \rightarrow \linMod(x_{t-1}) \rightarrow{\{ z_{t}, z_{t+1}, z_{t+2},\ldots\}} $ is invertible. $ {\{ z_{t}, z_{t+1}, z_{t+2},\ldots\}} \rightarrow \linMod(x_{t-1})^{-1} \rightarrow{\{ x_{t}, x_{t}, x_{t+1},x_{t+2}\ldots\}} $ and the inverse can be interpreted as giving the impact of ``fully anticipated future shocks'' on the path of $x_t$  in a linear perfect foresight model.  
...
Here, the series representation will provide a weighted sum of $z_t\tArg$ functions that give us an approximation for the, in this case, known model solution.
...
By including enough terms we can compute the exact solution for $x_t$ 
exactly.
...
\end{tcolorbox}
\noindent
substituting $\rcpC_{t+1}$ for $\frac{\theta_{t+1}}{c_{t+1}}$ in the first equation and 
...
These following coefficients  produce a unique stable linear solution.
...
results for the original variables.
...
Consider a bounded region, $\bndRgn$, that contains all iterated conditional expectations paths.
  By bounding the largest deviation in the paths for the $\Delta z_t^p$ we can bound the largest diviation in a proposed solution from the exact solution.
...
\begin{figure}
  \centering
  \includegraphics[width=3.0in]{ergodicKTheta.pdf}
  \includegraphics[width=3.0in]{ergodicZs.pdf}
...
\subsection{Error Boound Formula}
\label{sec:error-boound-formula}
