...
\title{A New Series Representation for Robustly Computing
  Nonlinear Dynamic Stochastic Model Solutions: 
A New Solution Algorithm and Error Formulas } %(Better?, Coherent? Comprehensive? General? )
...
\begin{abstract}
This paper provides a technique for solving discrete time  nonlinear dynamic stochastic
infinite-horizon economies with bounded-time-invariant solutions.
In addition, the paper provides 
formulae for evaluating the accuracy of proposed solutions
and presents
a robust algorithm for improving the accuracy of those solutions.
The error bounds apply for assessing the accuracy 
of  proposed bounded time invariant decision rules 
...
value function iteration but I will defer treating this topic for
a future work.
}
...
Section \ref{sec:solnerrorbounds} provides formulae for computing dynamic stochastic model error bounds for proposed  solutions.
...
saddle point models can play a very useful role in characterizing the 
...
\end{gather}
It will be useful to collect the components of this solution for use later in the paper. We will use these matrices to construct a series representation for any bounded path.
...
Note that the reference model is deterministic and the $z_t\tArg$ functions account for the stochastic nature of the model.
 The formula supports the intuition that far distant shocks
...
These coefficients are not completely arbitrary merely because the series 
representation requires that the linear model
has a unique stable solution.\footnote{I have chosen $H_1$ full rank to span the space for the 3 arbitrary independent $z_t$ time series.} The solution for this system is
}
  B=
...
representation requires only the $H_1$ spanning condition and the requirement that the state values along the 
...
x_{3,t}=\epsilon_t \intertext{and the $\epsilon_t$ are a sequence of pseudo random draws from the uniform distributions $\mathcal{U}(-4,4)$ produced subsequent to the selection of a random seed.} randomseed(\someNorm{x_{t-1}}+\someNorm{\epsilon_t})
\end{gather} 
...
Consequently, by applying the law of iterated expectations, 
we can compute conditional expected solution paths 
...
a series representation for the model solution solutions  and
to construct error bounds for the approximation.
...
Here, the series representation will provide a weighted sum of $z_t\tArg$ functions that give us an approximation for the model solutions.
...
In this section, to keep things simple, we will begin with a familiar RBC model example.\footnote{
...
\label{sec:rbcaux}
  We consider a model described in \cite{Maliar2005}\footnote{Here, we set their $\beta=1$ and 
...
For any given values of $k_{t-1},\theta_{t-1}, \epsilon_t$, the model decision rule, ($\DR$), and decision rule conditional expectation, ($\DRCE$), can generate a conditional expectations path for any given initial $x_{t-1}$ and $\epsilon_t$:
\begin{gather*}
  \xWOarg_t(\xtmEpsArg)=\DR(\xtmEpsArg)\\
  \xWOarg_{t+k+1}(\xtmEpsArg)=\DRCE(\xWOarg_{t+k}(\xtmEpsArg))
\end{gather*}
and produces paths for $\{(z_{1t}\tArg, z_{2t}\tArg, z_{3t}\tArg),\ldots\}$
\begin{gather*}
...
We can reconstruct a linear reference model for the modified RBC system
by  augmenting the RBC model with the equation substituting $\rcpC_{t+1}$ for $\frac{\theta_{t+1}}{c_{t+1}}$ 
\begin{tcolorbox}[ams gather]
...
We can construct an error bound for proposed model solutions.
Consider a bounded region, $\bndRgn$, that contains all iterated conditional expectations paths.
  By bounding the largest deviation in the paths for the $\Delta z_t^p$ we can bound the largest diviation in a proposed solution from the exact solution.( For proof see Appendix \ref{sec:error-bound-formula}).
...
% augment the model equations to include auxiliary variables so that we can 
...
\eqnFuncSysI{i}\equiv \eqnFuncSysIExpl{i} \label{eqnGates}
\end{gather}
 represents a set of model equations, $\eqnFunc_i$,  with a Boolean valued gate, $\preGate_i, \postGate_i$. Consequently, we will have an equation system and  corresponding gatekeeper logical expressions 
indicating which equation system is in force for producing the unique
solution for a give $x_{t-1}, \epsilon_t$.
  They represent exhaustive and mutually exclusive equation systems determining the solution at time t for the model.  They could be complementary slackness conditions or could represent a model with different regimes. We will consider time invariant model solutions $x_t=\xWarg$.  Given $\xWarg$, $\XWOarg(x_{t-1})\equiv \expct{\xWarg}$. 
...
Before presenting the pseudocode for the detailed algorithm, this subsection
characterizes  how the proposed approach differs from 
...
% \fi
% \iflinux
% \includegraphics[width=3.0in]{resDir-host-node006dnumKern16-3727705036/forBetterRBC--1,1,1-Iters70theK0Trad.pdf}
% \includegraphics[width=3.0in]{resDir-host-node006dnumKern16-3727705036/forBetterRBC--1,1,1-Iters70theK0TradApprox.pdf}
% \fi
...
% \fi
% \iflinux
% \includegraphics[width=3.0in]{resDir-host-node006dnumKern16-3727705061/forBetterRBC--1,1,1-Iters70theK0Series.pdf}
% \includegraphics[width=3.0in]{resDir-host-node006dnumKern16-3727705061/forBetterRBC--1,1,1-Iters70theK0SeriesApprox.pdf}
% \fi
...
$(k_{t-1},\theta_{t-1},\epsilon_t)$ generated using the Niederreiter algorithm
 covering the ergodic set.  I do this for values of k from 0 to 20.  The static proposed solution comes from linearing
the RBC model around the deterministic steady tate for
...
\begin{figure}
  \centering
  \label{fig:ccoefsk0}
  \caption{$c_{err}= c_0+ \beta c_{predicted}$}
\begin{gather}
...
To demonstrate this phenomonon, 
I experimented with an RBC model with occassionally binding constraints.
...
the linear perfect forsight solution to use as the linear reference model and
...
It should be possible to maintain a small footpring communication would involve
...
Although the series representation it appears much can be gained by using
just the first term.
...
\section{Error Bound Formula Proof}
...
To apply the series formula, one must construct condional expectations paths from each initial $x(x_{t-1},\epsilon_t)$.  Consider the case when the transition 
