...
\end{gather*}
Consequently, we can use the decision rule to compute values for $c_t$ and $k_t$ for any intial values for $k_{t-1}$ and $\theta_{t-1}$ and realization of $\epsilon_t$.
...
For example, using the following parameter values and using the linearized version of the model to generate the linear reference model.
...
The figure   \ref{rbcTrunc} shows the impact on the initial value of the state variables of truncating the series.   The bound again is very pessimistic. 
