...
{\small
  \begin{tcolorbox}[ams gather]
  \Delta z_t \le  
\max_{\{x_{-},\epsilon\}} \twoNorm{ \phiMat \eqnFunc(x_{-},g^p(x_{-},\epsilon),G^p(g^p(x_{-},\epsilon)),\epsilon) }\\
	\infNorm{ x^\star_{t}(x_{t-1},\epsilon) -	 x^p_{t}(x_{t-1},\epsilon)} \le
\max_{\{x_{-},\epsilon\}} \twoNorm{(I-F)^{-1} \phiMat \eqnFunc(x_{-},g^p(x_{-},\epsilon),G^p(g^p(x_{-},\epsilon)),\epsilon) }
  \end{tcolorbox}
}
...
$\xzFunc(x,\epsilon)=
\begin{bmatrix}
B x+(I-F)^{-1} \phiMat \psiCMat+ \psiEpsMat \epsilon_t\\0_{\numZ}  
\end{bmatrix}$\\
$\XZFunc(x,\epsilon)=
\begin{bmatrix}
...
$x_t=B x_{t-1}+ (I-F)^{-1} \phiMat \psiCMat + \phiMat . \psiEpsMat . \epsilon_t+
