...
k_{t-1}\\
\theta_{t-1}
  \end{bmatrix}  + \phi \psi_\epsilon\epsilon_t + (I - F)^{-1} \phi \psi_c + \sum_{\sForSum=0}^\infty F^s \phi z_{t+\sForSum}(k_{t-1},\theta_{t-1}, \epsilon_t) \label{theSeries}
\end{gather*}
% \footnote{
...
The first few terms make all the difference in approximating the value of the state variables.
...
Alternatively, for assessing the precision of the computation across an array of initial conditions, we can apply a variation on the formula \ref{pathErr}

    \begin{gather}
\infNorm{\xWarg-\xWargK} \le \infNorm{(I -F)^{-1} \phi \psi_z}  \max \left ( \infNorm{\Delta z_t } \right ) \label{pathErrMax}
    \end{gather}  
By identifying the largest value that we find for adjusting the $z_t$, we bound the maximum deviation that will impact the inital  $x_t$ values.
...
Choosing p wisely convergence slow until \cite{Niederreiter1983}.  They speed up the convergence by contracting the domain.
...
\end{gather}
 Represent a collection of models and boolean valued gates.  They represent exhaustive and mutually exclusive equation system determining the solution at time t for the model.  They could be complementary slackness conditions or could represent a model with different regimes. We will consider time invariant model solutions $x_t=\xWarg$.  Compute $\XWOarg(x_{t-1})\equiv \expct{\xWarg}$. We will have
...
\begin{description}
\item[Equation System] \ 
For example, the Euler equations for the  neoclassical growth  model 
\label{sec:simple-rbc-model-ext} can be written as
\begin{gather}
\mu>0 \land (k_t - (1-d)k_{t-1}-\upsilon I_{ss})=0=\mathbf{true}\\
  \lambda_t -\frac{1}{c_t}\\
...
\mu=0 \land (k_t - (1-d)k_{t-1}-\upsilon I_{ss})\ge 0=\mathbf{true}\\
  \lambda_t -\frac{1}{c_t}\\
c_t+k_t-\theta_tk_{t-1}^\alpha\\
N_t-\lambda_t \theta_t\\
\theta_t-e^{(\rho\ln(\theta_{t-1})+\epsilon)}\\
...
\end{gather}
 ...
 \item[genxkzk]
    \begin{gather}
      ...
      \end{bmatrix}= \mathcal{G}(x_{t-1},\epsilon_t,z_t|\linMod,\XtFunc(\cdot),x_t^g)
    \end{gather}
...
\label{sec:symbols}
\begin{gather}
\intertext{xgusst}%[1]{x^{guess}_{#1}}
\xgusst{x}
 \intertext{xnxt}%[1]{x^{next}_{#1}}
\xnxt{x}
 \intertext{znxt}%[1]{z^{next}_{#1}}
\znxt{x}
 \intertext{xlst}%[1]{x^{last}_{#1}}
\xlst{x}
 \intertext{zlst}%[1]{z^{last}_{#1}}
\zlst{x}
 \intertext{zgusst}%[1]{z^{guess}_{#1}}
\zgusst{x}
 \intertext{xguss}%{\xgusst{}}
\xguss
 \intertext{xtpguss}%{x^{tp1guess}}
\xtpguss
 \intertext{iSet}%{{\mathcal{I}_t}}
\iSet
 \intertext{posInt}%{{\mathcal{N}^+}}
\posInt
 \intertext{Rn}%[1]{{\mathcal{R}^{#1}}}
\Rn{x}
 \intertext{numX}%{{N_x}}
\numX
 \intertext{numZ}%{{N_z}}
\numZ
 \intertext{numEps}%{{N_\epsilon}}
\numEps
 \intertext{numR}%{{N_r}}
\numR
 \intertext{numIters}%{{K}}
\numIters
\intertext{numTerms}%{{N_{terms}}}
\numTerms
 \intertext{numPts}%{{N_{points}}}
\numPts
 \intertext{thePoints}%{{\{(x_{t-1},\epsilon_t)^{1} \ldots (x_{t-1},\epsilon_t)^{\numPts}\}}}
\thePoints
 \intertext{thePts}%{{\mathcal{P}}}
\thePts
 \intertext{xtmEpsArg}%{{x_{t-1},\epsilon_t}} 
\xtmEpsArg
 \intertext{xtArg}%{{x_{t} }}
\xtArg
 \intertext{sumLinPart}%{{
\sumLinPart
 \intertext{sumZPart}%{{
\sumZPart
 \intertext{sumZPartZero}%{{
\sumZPartZero
 \intertext{sumZPartPos}%{{
\sumZPartPos
 \intertext{EsumLinPart}%{{
\EsumLinPart
 \intertext{EsumZPartZero}%{{
\EsumZPartZero
 \intertext{EsumZPartEpsilon}%{{
\EsumZPartEpsilon
 \intertext{EsumCapZPart}%[1]{{
\EsumCapZPart{x}
 \intertext{uncnxpt}%[2]{{\mathcal{E}\left [ \left . #1 \right | #2  \right ]}}
\uncnxpt{x}{y}
 \intertext{expct}%[1]{{\mathcal{E}\left [ #1 \right ]}}
\expct{x}
 \intertext{xzFuncGuess}%{{\mathbb{g}}}
\xzFuncGuess
 \intertext{xzFuncGuessIn}%{{\mathbf{\mathbb{g}_{in}}}}
\xzFuncGuessIn
 \intertext{genxzFuncGuessIn}%{{\mathbf{gen\mathbb{g}_{in}}}}
\genxzFuncGuessIn
 \intertext{xzFuncGuessSig}%{{(\Rn{\numX})\rightarrow
\xzFuncGuessSig
 \intertext{xzFunc}%{{{\gamma}}}
\xzFunc
 \intertext{xzFuncSig}%{{(\Rn{\numX+\numEps})\rightarrow
\xzFuncSig
 \intertext{XZFunc}%{{\mathbb{G}}}
\XZFunc
 \intertext{genXZFunc}%{{\mathbf{gen\mathbb{G}}}}
\genXZFunc
 \intertext{bigXFuncSig}%{{(\Rn{\numX})\rightarrow
\bigXFuncSig
 \intertext{genXGFP}%{{\mathcal{U}}}
\genXGFP
 \intertext{xgFP}%{{\mathcal{V}}}
\xgFP
 \intertext{genFpf}%{{\mathbf{gen}_\fpf}}
\genFpf
 \intertext{fpf}%{{\mathcal{T}}}
\fpf
 \intertext{fpfIn}%{{\mathbf{t}_{in}}}
\fpfIn
 \intertext{fpfOut}%{{\mathbf{t}_{out}}}
\fpfOut
 \intertext{genFpfIn}%{{\mathbf{gen\fpf}_{in}}}
\genFpfIn
 \intertext{genFpfOut}%{{\mathbf{gen\fpf}_{out}}}
\genFpfOut
 \intertext{genSlvr}%{{\mathbf{gen}_{\slvr}}}
\genSlvr
 \intertext{slvr}%{{\mathcal{S}}}
\slvr
 \intertext{slvrIn}%{{\mathbf{s}_{in}}}
\slvrIn
 \intertext{slvrOut}%{{\mathbf{s}_{out}}}
\slvrOut
 \intertext{genSlvrIn}%{{\mathbf{gen\slvr}_{in}}}
\genSlvrIn
 \intertext{genSlvrOut}%{{\mathbf{gen\slvr}_{out}}}
\genSlvrOut
 \intertext{solverSig}
 \intertext{proj}%[1]{{\pi_{#1}}}
\proj{x}
 \intertext{eqnArg}%{\mathbf{m}_{in}}
\eqnArg
 \intertext{eqnOut}%{\mathbf{m}_{err}}
\eqnOut
 \intertext{eqnFunc}%{\mathbb{M}}
\eqnFunc
 \intertext{eqnErrs}%{{{m}_e}}
\eqnErrs
 \intertext{eqnFuncSig}%{{\Rn{3\numX+\numEps+\numZ}\rightarrow\Rn{\numX}}}
\eqnFuncSig
 \intertext{lilXFuncSig}%{{fix lilXFuncSig}}
\lilXFuncSig
 \intertext{frfpnsFuncSig}%{{\Rn{\numX+\numEps}\rightarrow\Rn{\numX+\numZ}}}
\frfpnsFuncSig
 \intertext{bigXRegimeFuncSig}%{{(\Rn{\numX+1})\rightarrow
\bigXRegimeFuncSig
 \intertext{lilXRegimeFuncSig}%{{(\Rn{\numX+1+\numEps+\numZ})\rightarrow
\lilXRegimeFuncSig
 \intertext{eqnRegimeFuncSig}%{{
\eqnRegimeFuncSig
\intertext{frfpnsRegimeFuncHelp}%%{{\Rn{\numX+\numEps}\rightarrow\Rn{\numX+\numZ}}}
\frfpnsRegimeFuncHelp
 \intertext{frfpnsRegimeFuncSig}%{{
\frfpnsRegimeFuncSig
 \intertext{dstSpec}%{{\mathbf{distSpec}}}
\dstSpec
 \intertext{expctSpec}%{{\mathbf{expctSpec}}}
\expctSpec
 \intertext{grdSpec}%{{\mathbf{grdSpec}}}
\grdSpec
 \intertext{linMod}%{{\mathcal{L}}}
\linMod
 \intertext{linModMats}%{{\{H,\psi_\epsilon,\psi_c;B,\phi,F\}}}
\linModMats
 \intertext{sForSum}%{{\nu}}
\sForSum
 \intertext{xWOarg}%{   \mathcal{X}_{t}}
\xWOarg
 \intertext{xWarg}%{   \mathcal{X}_{t}(x_{-1},\epsilon)}
\xWarg
 \intertext{xWargK}%{   \hat{\mathcal{X}}_{t}(x_{-1},\epsilon,k)}
\xWargK
 \intertext{xWOargK}%{   \hat{\mathcal{X}}_{t}}
\xWOargK
 \intertext{xIter}%[2]{\mathcal{X}^{#1}(#2)}
\xIter{x}{y}
\intertext{zWOarg}
\zWOarg
\intertext{zWarg}
...
\intertext{zWOargK}
\zWOargK
 \intertext{xNow}%[1]{x^{#1}_t(x_{t-1},\epsilon_t)}
\xNow{x}
 \intertext{zNow}%[1]{z^{#1}(x_{t-1},\epsilon_t)}
\zNow{x}
 \intertext{xNowtp}%[1]{x^{#1}_{t+1}(x_{t-1},\epsilon_t)}
\xNowtp{x}
 \intertext{XNow}%[3]{\mathcal{X}^{#1}_{#2}(x_{#3})}
\XNow{x}{y}{z}
 \intertext{ZNow}%[3]{\mathcal{Z}^{#1}(x_{#3})}
\ZNow{x}{y}{z}
 \intertext{rcpC}%{{\mathbf{N}}}
\rcpC
 \intertext{gssaKtp}%{{\left \{  
\gssaKtp
\intertext{QED}
\myQed\intertext{xtfuncti}
\xtFuncTI
\intertext{Xtfuncti}
\XtFuncTI\intertext{expctEps}
\expctEps{x}\intertext{xsubtfunc}
\xsubtFunc{x}{y}
\intertext{xtFunc}
\xtFunc{x}
\intertext{XtFunc}
