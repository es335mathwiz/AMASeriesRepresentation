...
Nonlinear Dynamic Stochastic Model Solutions: Comprehensive(General) Error Bounds and a New Solution Algorithm}
...
\begin{abstract}
  This paper proposes a new series representation for bounded discrete time series paths. Applying this series representation
  to model solution specific conditional expectations paths generates
  exploitable constraints on these model solution decision rules.
  One can use this series representation to
  construct error bounds for proposed solutions for a very broad class of dynamic stochastic models.
...
proposed model solution, regardless of the source, for a wide class of dynamic models including models with occasionally binding constraints, and regime switching.
...
The paper also proposes a new algorithm that augments traditional solution procedures
with these additional solution constraints.
%As in the traditional case, series representation algorithm improves upon an initial guess for a stochastic model solution by solving a potentially complicated, but deterministic problem in the initial time period.
As a result, the solution procudure accomodates models with multiple invariant spaces and provides a framework for applying tools from dynamical systems theory for determining and maintaining boundaries for these invariant spaces.
Like the error bound formula, the algorithm is appropriate for models with occasionally binding constraints and regime switching. 


The  paper uses a particular implementation of the algorithm to
demonstrate how to use the 
series representation in conjunction with 
Smolyak polynomial function approximation  to exploit the high degree of parallelism available in the algorithm and to eliminate the need for numerical integration during the solution updates.
...
used to generate a transformation of one infinite dimensional series into another.
...
\begin{itemize}
\item Time series paths as components of dynamic models
\item Potter impulse response functions
\item $\phi=0$ is standard
...
including, but not restricted to Euler errors,  to construct an error bound for the norm of the distance between the proposed and the exact solution.  This work complements the analysis provided in
...
Under certain rank conditions, this transformation is one to one and onto.\footnote{$\psi_c$ full rank provides a sufficient condition, but not necessary condition.}  The
...
certain rank conditions. $ {\{ z_{t}, z_{t+1}, z_{t+2},\ldots\}} \rightarrow \linMod(x_{t-1})^{-1} \rightarrow{\{ x_{t}, x_{t}, x_{t+1},x_{t+2}\ldots\}} $ and the inverse can be interpreted as giving the impact of ``fully anticipated future shocks'' on the path of $x_t$ obeying the solution of a linear rational expectations model.  
...
\begin{gather}
  x_{1,t}=\alpha D_\pi(t) \intertext{where $D_\pi(t)$ gives the t-th digit of $\pi$}
x_{2,t}=\beta (-1)^t\\
x_{3,t}=\epsilon_t \intertext{ and the $\epsilon_t$ are a sequence of pseudo random draws from the uniform distributions $\mathcal{U}(-4,4)$}
\end{gather} 
Figure \ref{arbpaths} displays these three trajectories. In  the first set of trajectories characterizes 
...
to compute the impact on the current state of fully anticipated future shocks.  The formula characterizes the impact exactly.  However one can contemplate the impact of at least two types of incorrect.  One could truncate a series of correct values of $z_t$.  One might have imprecise values of $z_t$ along the path.
...
For an overview see \cite{judd92,Christiano2000,doraszelskiy04,gaspar97,Judd2014}
...
\includegraphics[width=2.7in]{simpArbBoundsVActual.pdf}  
  \caption{RBC Truncation Error Bound Versus Actual: Arbitrary Linear Reference Model}
  \label{rbcTrunc}
\end{figure}
\begin{figure}
  \centering
\includegraphics[width=2.7in]{simpBoundsVActual.pdf}  
  \caption{RBC Truncation Error Bound Versus Actual: Stochastic Steady State Linearization Linear Reference Model}
  \label{rbcTruncSimp}
\end{figure}
