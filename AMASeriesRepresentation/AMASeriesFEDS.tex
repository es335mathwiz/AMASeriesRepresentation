...
model solution.\footnote{This work contrasts with the analysis provided in
...
fixed in the experiment, the
  proposed decision rules and the linear reference model both change.

Figures \ref{fig:forc} and \ref{fig:fork}  present the 100 $R^2$ and estimated variance and Figures \ref{fig:forcn} and \ref{fig:forkn} presents the 100 resulting regression coefficients for a variety of values for $K$, of number of terms in the 
...
constant tends to be close to zero.
...
boolean function postcondition for a given equation system. If some $\preGate_i$ is false, then there is no need to try to solve model $i$. If $\postGate_i$ is false, the system has produced an unacceptable solution which should be ignored.
...
Fewer means more truncation error. 
...
the  usual technique for solving theset types of models.
...
Table \ref{evalpA} shows the evaluation points when the degree of approximation
was (1,1,1).  Table \ref{valatA} shows the decesion rule prescription for $(c_t,k_t,\theta_t)$ at each evaluation point. Table \ref{acterrA} shows the actual error in the decision rule
...
%\vspace{.2in}
   ...
   \input{errByKTab}     
   \end{gather*}
   \caption{RBC Known Solution: Impact of Series Length on Approximation Error}
 \end{table}
