...
Smolyak polynomial function approximation to completely
avoid numerical integration in stochastic models
...
Section \ref{sec:newseries} presents a usefule new series representation for any bounded time series.
...
A key feature to note is that the series representation can accomodate arbitrarily complex time series trajectories, so long as these trajectories are bounded.
...
\subsection{An Example: An ``Almost'' Arbitrary Linear Model and an Some Arbitrary  Bounded Time Series}
...
\includegraphics[width=3in]{theZs.pdf}  
  ...
  \caption{Arbitrary Bounded Time Series Paths and Corresponding $z_{i,t}$ values}\label{pipath}
\end{figure}
