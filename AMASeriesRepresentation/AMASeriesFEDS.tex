...
for bounded discrete time series applicable for
...
augments traditional solution procedures with additional constraints thereby
enhancing algorithm robustness and improving accuracy.
The technique can readily accomodate models with multiple invariant spaces.
...
in handling dynamic economies with occasionally binding constraints\citep{holden15:_exist_dsge,guerrieri15:_occbin}. For an overview of solution techniques,
...
one bounded infinite dimensional series into another.
    ...
    errors in the decision rule without resort to ancillary simulations
...
paralletope method\citep{Judd2014}
...
of the algorithms components can be computed in parallel.
...
representation for the entire family of trajectories. Numerical linear algebra calculationscan easily transform a $z_t$ series into an $x_t$ series and vice versa.
...
Consider a bounded region, $\bndRgn$, where all iterated conditional expectations paths remain bounded.
 Given an exact solution $x^\star_t=g^\star(x_{t-1},\epsilon_t)$ define te conditional expectations function,
  \begin{gather}
...
\sum_{\sForSum=0}^\infty F^s \phi \infNorm{\Delta z_{t+\sForSum}^p(x_{t-1},\epsilon_t)}    
  ...
  By bounding the largest deviation in the paths for the $\Delta z_t^p$ we can bound the largest difference in $x_t$.\footnote{Since the future values are probabality weighted averages of the $\Delta z_t^p$ values for the given initial conditions and the condition expectations paths remain in the region $\bndRgn^p$,
...
describes in how the proposed approach differs from 
...
compares the various approximations to the known actual solution.
...
$\expct{x_{t+1}}$ in the model equations.  Typically  20 iteratation are more than enough for the standard and the new approach.
...
The following graphs have present key characterics of the solutions.  
The panels labelled approximation error show contour plots of $\mathbb{E}(z_{1,t-1},z_{2,t-1})$:
\begin{gather*}
\mathbb{E}(z_{1,t-1},z_{2,t-1})\equiv \sqrt{(c_t()-c_t^\star())^2 +(k_t()-k_t^\star())^2 +(\theta_t()-\theta_t^\star())^2 +(N_t()-N_t^\star())^2 }  
\end{gather*}
The panels labelled equation error show  contour plots for
\begin{gather*}
  \mathbb{D}(z_{1,t-1},z_{2,t-1})\equiv \eqnFunc(x_{-},g^p(x_{-},\epsilon),G^p(g^p(x_{-},\epsilon)),\epsilon).
\end{gather*}
...
\ifmacosx
  ...
  \includegraphics{resDir/forBetterRBC--1,1,1-Iters20theK0host-MacBook-Pro-2numKern2Trad.pdf}


  \includegraphics{resDir/forBetterRBC--1,1,1-Iters20theK0host-MacBook-Pro-2numKern2ApproxTrad.pdf}
\fi
\iflinux
...
\ifmacosx
\input{resDir/forBetterRBC--1,1,1-Iters20theK20host-MacBook-Pro-2numKern2errBnd.tex} 
\fi
is 25\% larger than the actual error 
\iflinux
\input{resDir/forBetterRBC--1,1,1-Iters20theK20host-MacBook-Pro-2numKern111acterr.tex} 
\fi
...
\begin{figure}
  \centering
\ifmacosx
  \includegraphics{resDir/forBetterRBC--2,2,2-Iters20theK20host-MacBook-Pro-2numKern2ApproxTrad.pdf}


  \includegraphics{resDir/forBetterRBC--2,2,2-Iters20theK20host-MacBook-Pro-2numKern2.pdf}
\fi
...
\subsection{Ocasionally Binding Constraints}
\label{sec:occbind}
