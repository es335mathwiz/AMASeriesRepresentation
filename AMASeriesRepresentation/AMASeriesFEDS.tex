...
\marginpar{Address Bullets in Other Computing? Section}
Using this new series representation, the paper 
augments the traditional solution strategy with an additional set of
...
This paper addresses the solution of discrete time  dynamic stochastic
...
one bounded infinite dimensional series into another.
    ...
    decision rules.\footnote{    Others have also studied approximation error for dynamic stochastic models\cite{judd2017lower,santos2005accuracy,Santos2000accuracy}.} This paper provides  approximation
...
and precomputes all integrals required by the calculations.
...
\label{sec:newseries}
Since \cite{blanchard80} numerous authors have developed solution algorithms and formulas for solving linear rational expectations models.  It turns out that
...
between {\em linear reference models} and bounded time series in preparation for
...
\end{gather}
It will be useful to collect the components of this solution,$\linMod \equiv \linModMats$, for use later in the paper.\footnote{In Section \ref{sec:almostarbitrary}, I will use these matrices  to construct a series representation for any bounded path.}
...
its column space will span the column space for all non zero values for $z_t \in \mathcal{R}^3$.  The flexibility in choosing $\linMod$ will 
become more important in the context below where we must 
choose a  linear reference model in a context where we may have 
little guidance about what might make a good choice.  It turns out that the algorithm is very forgiving about the choice of $\linMod$.  
...
a z series exactly replicating any set of trajectories.  Thus, the family
of z functions along with equation \ref{theSeries} can provide a series 
representation for the entire family of trajectories. Consequently, 
numerical linear algebra calculations can easily transform a $z_t$ series into an $x_t$ series and vice versa.  This can be done for any path starting from any particular initial conditions. 
 Thus,  for example, any discrete time invariant map that generates families of bounded paths has a series representation.
...
Figure \refeq{rbcpaths} shows, from top to bottom, the paths of $\theta_t, c_t, \text{ and } k_t$ from the initial values given in Equation \refeq{theInits}.  The right panel shows the paths for the $z_t$ variables associated with the linear reference model. 
...
This section reports an experiment evaluating how well several different
formulations of the error approximation perform.  
I compute the known decision rule for the RBC model with parameters
$\{\alpha=0.36,\delta=0.95,\eta=1,\rho=0.95,\sigma=0.01\}$.  
I generate a 10 other settings for the model parameters by choosing
...
I generate 30 representative points, $(k_{i,t-1},\theta_{i,t-1},\epsilon_{i,t})$, from the ergodic set for the reference model
using the Niederreiter quasi-random algorithm.
To approximate the error I use the proposed decision rule and decision rule conditional expectation to generate a path forward $k$ steps beyond time $t$ 
for each representative point.  I then evaluate the
model equations at each point along that path.  This produces a sequence of 
errors that approximate how much the values of z's would need to change 
and consequently, appropriately weighted, the quantity $\hat{\mathbf{e_i}}$,
approximately how much the value of
$x_t$ needs to change.  I regress the actual error on the predicted error 
computed at the 30 representative points. 


\begin{gather*}
  \mathbf{e_i}=\alpha + \beta   \hat{\mathbf{e_t}} +u_i
\end{gather*}

 
I do this for each of the 100 possible 
combinations of the, by inaccurate by construction, 
 10 linearizations and the 10 linear reference models.
The reference model for the true solution stays fixed while the proposed
...
proposed decision rule function.\footnote{The formula predicts the changes 
...
\marginpar{incorporate bullets into text}
\begin{enumerate}
...
of the actual, multi-system implementation follows.
\begin{gather*}
 \eqnFunc(x_{t-1},x_t,\expct{x_{t+1}},\epsilon)=0.\intertext{  We seek }
\eqnFunc(x_{t-1},\xWOarg^\star\tArg,\XWOarg^\star(\xWOarg^\star\tArg),\epsilon)=0\,\,\forall \tArg.  \intertext{Given a proposed model solution}
...
the  traditional technique or the approach suggested here.
...
\tcc{Given approximation levels,  PCA information, and probability distributions for errors compute the collocation points, the Psi Matrix, the polynomials and the integrals olf the polynomials}
\Output{$\{xPts,PsiMat,polys, intPolys\}$}
\caption{$smolyakInterpolationPrep$\label{smolyakInterpolationPrep}} 
\end{algorithm}
...
\tcc{Given approximation levels, variable ranges and probability distributions for errors compute the collocation points, the Psi Matrix, the polynomials and the integrals olf the polynomials}
\Output{$\{xPts,PsiMat,polys, intPolys\}$}
\caption{$smolyakInterpolationPrep$\label{smolyakInterpolationPrep}} 
\end{algorithm}
\clearpage
...
\tcc{Given approximation levels, variable ranges and probability distributions for errors compute the collocation points, the Psi Matrix, the polynomials and the integrals olf the polynomials}
\Output{$\{xPts,PsiMat,polys, intPolys\}$}
\caption{$\label{genSolutionErrXtEps}$\label{genSolutionErrXtEps}} 
\end{algorithm}
...
\tcc{Given approximation levels, variable ranges and probability distributions for errors compute the collocation points, the Psi Matrix, the polynomials and the integrals olf the polynomials}
\Output{$\{xPts,PsiMat,polys, intPolys\}$}
\caption{$\label{genSolutionErrXtEpsZero}$\label{genSolutionErrXtEpsZero}} 
\end{algorithm}
...
\tcc{Given approximation levels, variable ranges and probability distributions for errors compute the collocation points, the Psi Matrix, the polynomials and the integrals olf the polynomials}
\Output{$\{xPts,PsiMat,polys, intPolys\}$}
\caption{$\label{genSolutionPath}$\label{genSolutionPath}} 
\end{algorithm}
