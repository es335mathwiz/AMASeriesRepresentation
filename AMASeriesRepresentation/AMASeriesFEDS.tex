...
Boolean function post-condition for a given equation system. If some $\preGate_i$ is false, then there is no need to try to solve model $i$. If $\postGate_i$ is false, the system has produced an unacceptable solution which should be ignored.
...
Each of the estimated errors were computed with $K=5$.
