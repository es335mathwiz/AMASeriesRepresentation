...
constraints thereby enhancing algorithm reliability
...
its column space will span the column space for all non zero values for $z_t \in \mathcal{R}^3$.\footnote{The flexibility in choosing $\linMod$ will 
become more important in later in the paper  where we must 
...
\label{sec:rbcaux}
  Consider the model described in \citep{Maliar2005}\footnote{Here, I set their $\beta=1$ and 
...
\end{gather}
Recomputing the truncation errors with the expanded model produces nearly identicle results for  variable approximation errors.
...
Given a proposed model solution define 
the error in $x_t$ at $\tArg$

\begin{gather*}
\mathbf{e}\tArg\equiv \xWOarg^\star\tArg-\xWOarg^p\tArg\intertext{compute the conditional expectations functions for the decision rule and use them to generate a path to use with a linear reference model $\linMod  \equiv \linModMats$  in \ref{theSeries} to appoximate  $\mathbf{e}$.} 
...
fixed while the, by construction inaccurate  proposed
decision rules and the linear reference model  both change.

Figures \ref{fig:forc} and \ref{fig:fork}  present the $R^2$ and estimated variance and Figures \ref{fig:forcn} and \ref{fig:forkn} the regression coefficients for $k=0, 1, 10, 30$\footnote{The formula predicts the changes 
in $\theta_t$ perfectly since it is determined by a backward looking equation.}
The formula does a good job of predicting the error in the 
...
ooccasionally binding  constraints and models with regime switching.
...
boolean function postcondition for a given equation system. If some $\preGate_i$ is false, then there is no need to try to solve model $i$. If $\postGate_i$ is false, the system has produced an unacceptable solution which should be ignored.  Each of these individual models could be solved in parallel.   The
...
the conditional expectations function needed for computing conditional expectations paths from each collocation point forward.  
...
One must  specify a linear reference model.  One must choose a value for
$K$ the number of terms in the series representation.  There are tradeoffs.
More terms corresponds to more accuracy when the decision rule is accurate but
More terms is computationally more expensive. The initial guess is 
...
too sparse, increasing K will have less of an impact. 
...
Table \ref{evalp} shows the evaluation point when the degree of approximation
was (1,1,1).  Table \ref{acterr} shows the actual error in the decision rule
at each evaluation point.  Table \ref{esterr} shows the 
estimated error in the decision rule
...
conditional expectation paths relate to the time t solution values.
